module Hbci
  module Services
    class BaseReceiver < Base
      ...
      def hikazs
        @connector.session_service_response.hnvsd_data_block.find_segments('HIKAZS')
      end

      # Nachrichtenkopf version 3
      def build_hnhbk_version_3(posistion)
        Segment.new.head('HNHBK', posistion, 3).init(nil, 300, @connector.dialog_service_response.hnhbk[4].to_s, @connector.message_number)
      end
