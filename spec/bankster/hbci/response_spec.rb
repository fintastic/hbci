...
describe Bankster::Hbci::Response do
  let(:dialog)       { double }
  ...
  describe '.parse' do
    ...
    it 'contains a head' do
      expect(subject.head).to be_a(Bankster::Hbci::Segments::HNHBKv3)
    end

    it 'contains a tail' do
      expect(subject.tail).to be_a(Bankster::Hbci::Segments::HNHBSv1)
    end
