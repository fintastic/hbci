...
describe HbciNg::Services::TransactionsReceiver, type: :service do
  let(:from) { Time.now }
  let(:to) { Time.now }
  ...
  let(:sample) do
    sample = <<~HBCI
      HNHBK:1:3+000000000491+300+ID9120615224563+2'
      ...
      HNVSD:999:1+@299@
        ...
        HKKAZ:3:7+DE05740900000011111111:GENODEF1PA1:11111111::280:74090000+N+20190925+20190925'
        HKTAN:4:6+4+HKKAZ'
        HNSHA:5:2+TEST-SECURITY-REFERENCE++TEST-PIN'
      '
      HNHBS:7:1+1'
    HBCI
    sample.delete("\n").delete("\s")
