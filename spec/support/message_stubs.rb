...
def stub_dialog_init_request_message(credentials, rand: '10999990')
  ...
  lines = []
  lines << "HNHBK:1:3+000000000353+300+0+1'"
  lines << "HNVSK:998:3+PIN:1+998+1+1::0+1:#{date}:#{time}+2:2:13:@5@NOKEY:6:1+280:#{credentials.bank_code}:#{credentials.user_id}:V:1:1+0'"
  lines << "HNVSD:999:1+@195@HNSHK:2:4+PIN:1+999+#{rand}+1+1+1::0+1+1:#{date}:#{time}+1:999:1+6:10:16+280:#{credentials.bank_code}:#{credentials.user_id}:S:0:0'"
  lines << "HKIDN:3:2+280:#{credentials.bank_code}+#{credentials.user_id}+0+1'"
  lines << "HKVVB:4:3+0+0+1+Bankster+#{Bankster::Hbci::VERSION}'"
  lines << "HNSHA:5:2+#{rand}++#{credentials.pin}''"
  lines << "HNHBS:6:1+1'"
  lines.join
end
...
def stub_balance_request_message(credentials, account_number: '11111111', dialog_id: 'LM6022214510276', rand: '10999990')
  ...
  lines = []
  lines << "HNHBK:1:3+000000000334+300+#{dialog_id}+2'"
  lines << "HNVSK:998:3+PIN:1+998+1+1::0+1:#{date}:#{time}+2:2:13:@5@NOKEY:6:1+280:#{credentials.bank_code}:#{credentials.user_id}:V:1:1+0'"
  lines << 'HNVSD:999:1+@162@'
  lines << "HNSHK:2:4+PIN:1+942+#{rand}+1+1+1::0+1+1:#{date}:#{time}+1:999:1+6:10:16+280:#{credentials.bank_code}:#{credentials.user_id}:S:0:0'"
  lines << "HKSAL:3:4+#{account_number}:280:#{credentials.bank_code}+N'"
  lines << "HNSHA:4:2+#{rand}++#{credentials.pin}''"
  lines << "HNHBS:5:1+2'"
  lines.join
end
...
def stub_balance_response_message(credentials, account_number: '11111111', dialog_id: 'LM6022214510276', rand: '10999990')
  ...
  lines = []
  lines << "HNHBK:1:3+000000000485+300+#{dialog_id}+3+#{dialog_id}:3'"
  lines << "HNVSK:998:3+PIN:1+998+1+2::0+1:#{date}:#{time}+2:2:13:@5@NOKEY:6:1+280:#{credentials.bank_code}:#{credentials.user_id}:V:1:1+0'"
  lines << "HNVSD:999:1+@295@HNSHK:2:4+PIN:1+942+#{rand}+1+1+2::0+1+1:#{date}:#{time}+1:999:1+6:10:16+280:#{credentials.bank_code}:#{credentials.user_id}:S:0:0'"
  lines << "HIRMG:3:2+0010::Nachricht entgegengenommen.'"
  lines << "HIRMS:4:2:3+0020::*Abfrage der Kontosalden erfolgreich.'"
  lines << "HISAL:5:4:3+#{account_number}:280:#{credentials.bank_code}+Kontokorrent+EUR+C:42028,3:EUR:20160219'"
  lines << "HNSHA:6:2+#{rand}''"
  lines << "HNHBS:7:1+3'"
  lines.join
end
...
def stub_transactions_v6_request_message(credentials, account_number: '11111111', dialog_id: 'LM6022214510276', rand: '10999990', start_date: Date.new(2016, 2, 18), end_date: Date.new(2016, 2, 20))
  ...
  lines = []
  lines << "HNHBK:1:3+000000000353+300+#{dialog_id}+2'"
  lines << "HNVSK:998:3+PIN:1+998+1+1::0+1:#{date}:#{time}+2:2:13:@5@NOKEY:6:1+280:#{credentials.bank_code}:#{credentials.user_id}:V:1:1+0'"
  lines << 'HNVSD:999:1+@181@'
  lines << "HNSHK:2:4+PIN:1+942+#{rand}+1+1+1::0+1+1:#{date}:#{time}+1:999:1+6:10:16+280:#{credentials.bank_code}:#{credentials.user_id}:S:0:0'"
  lines << "HKKAZ:3:6+#{account_number}::280:#{credentials.bank_code}+N+#{start_date.strftime('%Y%m%d')}+#{end_date.strftime('%Y%m%d')}'"
  lines << "HNSHA:4:2+#{rand}++#{credentials.pin}''"
  lines << "HNHBS:5:1+2'"
  lines.join
end

def stub_transactions_v7_request_message(credentials, account_number: '11111111', dialog_id: 'LM6022214510276', rand: '10999990', start_date: Date.new(2016, 2, 18), end_date: Date.new(2016, 2, 20), attachment_id: nil, message_number: 2)
  date = Time.now.strftime('%Y%m%d')
  time = Time.now.strftime('%H%m%S')
  ...
  payload << if attachment_id.nil?
               "HKKAZ:3:7+#{iban}:#{bic}:#{account_number}::280:#{credentials.bank_code}+N+#{start_date.strftime('%Y%m%d')}+#{end_date.strftime('%Y%m%d')}'"
             else
               "HKKAZ:3:7+#{iban}:#{bic}:#{account_number}::280:#{credentials.bank_code}+N+#{start_date.strftime('%Y%m%d')}+#{end_date.strftime('%Y%m%d')}++#{attachment_id}'"
             end
